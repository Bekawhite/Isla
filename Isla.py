    ...
    for _, row in df.iterrows():
        ...
        if len(parts) >= 3:  # At least date, description, amount
            ...
            if len(parts) > 3:
                ...
                for part in desc_parts[:]:  # Use slice copy to avoid modification during iteration
        ...
        for idx, col in enumerate(selected_columns[:4]):
            with cols[idx % 4]:
                if col in column_analysis and column_analysis[col]:
                    # Safely convert column name to string and truncate
                    col_str = str(col) if col is not None else ""
                    if col_str:  # Only show if we have a valid string
